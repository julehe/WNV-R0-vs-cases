...
plot_gam_conf_all_f <- function(df, model, predictor, r0_predictor = F, type = "monthly",
                       binary = F, n_bins = 30){
  ...
  if(r0_predictor){
    new_data <- data.frame(
      ...
      median(df$perm_irrigated),
      median(df$percent_65plus),
      median(df$rain_3mo_cum)
    )
  }else{
    new_data <- data.frame(
      seq(10,
  ...
  names(new_data) <- c(predictor, "urban_fabric", "gdp_per_capita",
                       "perm_irrigated", "percent_65plus", "rain_3mo_cum")
